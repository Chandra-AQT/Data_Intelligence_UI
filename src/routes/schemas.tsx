...
import { Brain, Database, Eye, Pencil, Plus, Trash2, Upload } from "lucide-react";
      ...
      <div className="space-y-5">
        ...
        {isLoading ? (
          ...
          <div className="py-12 text-center">
            ...
            <p className="mt-3 text-muted-foreground">No schemas yet. Create one or upload a JSON file.</p>
          </div>
        ) : (
          <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
            {schemas.map((schema: { id: string; name: string; description: string; field_count: number; domain: string; created_at: string; fields?: Array<{ name: string; type: string; required: boolean }> }) => (
              <article key={schema.id} className="rounded-xl border border-border bg-card p-5 shadow-sm transition-transform hover:-translate-y-1 overflow-hidden">
