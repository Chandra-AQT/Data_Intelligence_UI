...
function Login() {
  ...
  return (
    <AuthFrame title="Sign in" subtitle="AQT Data Intelligence Â· Metadata Extractor">
      <form onSubmit={submit} className="space-y-4">
        ...
        <p className="text-center text-sm text-muted-foreground">
          Don't have an account? <Link to="/register" className="font-bold text-primary">Register â†’</Link>
        </p>
      </form>
    </AuthFrame>
  );
...
function AuthFrame({ title, subtitle, children }: { title: string; subtitle: string; children: React.ReactNode }) { return <div className="cockpit-bg flex min-h-screen items-center justify-center px-4"><div className="w-full max-w-md rounded-2xl border border-panel-border bg-card p-7 shadow-cockpit"><div className="mb-6 text-center"><Link to="/" className="text-3xl font-black text-primary">AQT</Link><p className="font-bold text-foreground">Data Intelligence</p><p className="text-sm text-accent">Metadata Extractor</p><h1 className="mt-6 text-2xl font-black text-foreground">{title}</h1><p className="text-sm text-muted-foreground">{subtitle}</p></div>{children}</div></div>; }
