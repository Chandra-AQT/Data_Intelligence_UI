...
function Login() {
      ...
      <AuthFrame title="Reset Password" subtitle="Enter your email to get reset instructions">
        {forgotSent ? (
          ...
          <div className="space-y-4">
            ...
            <button onClick={() => setForgotMode(false)} className="w-full text-sm text-center"
  ...
  return (
    <AuthFrame title="Sign in" subtitle="AQT Data Intelligence - Metadata Extractor">
      <form onSubmit={submit} className="space-y-4">
        ...
        <label className="grid gap-1 text-sm font-bold">
          ...
          <div className="relative">
            <input required name="password" type={show ? "text" : "password"}
              className="h-11 w-full rounded-lg border border-input bg-card px-3 pr-10" />
        ...
        <div className="flex justify-end">
          <button type="button" onClick={() => setForgotMode(true)} className="text-xs hover:underline"
            style={{ color: "#60a5fa" }}>
            Forgot password?
          </button>
        </div>
...
function AuthFrame({ title, subtitle, children }: { title: string; subtitle: string; children: React.ReactNode }) {
  return (
    <div className="cockpit-bg flex min-h-screen items-center justify-center px-4">
      <div className="w-full max-w-md rounded-2xl border border-panel-border bg-card p-7 shadow-cockpit">
        <div className="mb-6 text-center">
          <Link to="/" className="text-3xl font-black text-primary">AQT</Link>
          <p className="font-bold text-foreground">Data Intelligence</p>
          <p className="text-sm text-accent">Metadata Extractor</p>
          <h1 className="mt-6 text-2xl font-black text-foreground">{title}</h1>
